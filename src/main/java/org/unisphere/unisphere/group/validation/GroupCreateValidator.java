...
public class GroupCreateValidator implements
		ConstraintValidator<GroupCreateValidation, GroupCreateRequestDto> {

	private static final String GROUP_NAME = "^[a-zA-Z0-9ㄱ-ㅎㅏ-ㅣ가-힣]+$";
	private static final Pattern GROUP_NAME_PATTERN = Pattern.compile(GROUP_NAME);
	private static final int GROUP_NAME_MIN_LENGTH = 2;
	private static final int GROUP_NAME_MAX_LENGTH = 16;
	private static final List<String> ALLOWED_IMAGE_TYPES = Arrays.asList(
			"jpeg", "jpg", "png");

	private boolean validateName(GroupCreateRequestDto value, ConstraintValidatorContext context) {
		if (value.getName() == null || value.getName().isEmpty()) {
			context.buildConstraintViolationWithTemplate("그룹명을 입력해주세요.")
	...
	private boolean validateLogoImageUrl(GroupCreateRequestDto value,
	...
	@Override
	public boolean isValid(GroupCreateRequestDto value, ConstraintValidatorContext context) {
		context.disableDefaultConstraintViolation();
		return validateName(value, context) && validateLogoImageUrl(value, context);
	}
}
